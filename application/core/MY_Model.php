...
class MY_Model extends CI_Model
{
	...
	public function like($field, $match = '', $side = 'both', $escape = NULL)
	{
		if(is_array($field))
		{
			...
			foreach ($field as $key => $value)
			{
				if( $i == 0 )
				{
					$this->_database->like($key, $value, $side, $escape);
				}
				else
				{
					$this->_database->or_like($key, $value, $side, $escape);
				}
				$i++;
			}
			return $this;
	...
	{
		$campos = $this->_database->query('DESC '.$this->_table)->result();
		$tmp = array();
	...
	public function get($primary_value=null)
	{
		if($primary_value)
		{
			...
			{
				return $this->get_many_by($primary_value);
			}
			else
			{
				return $this->get_by($this->primary_key, $primary_value);
