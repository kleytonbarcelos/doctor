...
<script type="text/javascript">
	...
	{
		return [
			'<a title="Ver Usuário" data-toggle="tooltip" class="btn btn-default btn-xs" href="'+base_url_controller+'visualizar/'+row.id+'"><i class="fa fa-vcard"></i></a>',
			'&nbsp;',
	...
	function cadastrar()
	{
		$('#modalUsuarios .modal-title').html('<strong><i class="fa fa-plus-square"></i>&nbsp;Cadastro de Usuário</strong>');
	...
	function editar(id) //Preenche os campos do formulário apartir do DB
	{
		...
		$('#modalUsuarios .modal-title').html('<strong><i class="glyphicon glyphicon-edit"></i>&nbsp;Editar Usuário</strong>');
		$.ajax(
		{
	...
	$(function()
	{
		$('#modalUsuarios').on('hide.bs.modal', function(e)
		{
			clear_inputs_modal();
		});
		$('#formUsuarios').on('submit', function(event)
		{
			...
			.done(function(data) //success
			{
				...
				if(data.status == 1)
				{
					//alert_success('.msg', data.msg);
					alertify.success('<i class="fa fa-check-circle-o"></i> '+data.msg);
					$('#tableUsuarios').bootstrapTable('refresh', {url: base_url_controller+'bootstrap_table'});
