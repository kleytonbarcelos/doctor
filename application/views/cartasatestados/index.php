<div class="msg"></div>
<div id="toolbar">
	<button class="btn btn-sm btn-primary" onclick="window.location.href=base_url_controller+'cadastrar';"><i class="fa fa-plus"></i> aaaaaCadastrar</button>
	<button class="btn btn-sm btn-danger btnExcluir" disabled><i class="fa fa-trash"></i> Excluir</button>
</div>
<table
	id="tableCartasatestados"
	data-toggle="table"
...
<script type="text/javascript">
	...
	//#######################################################################################################################
	var $table = $('#tableCartasatestados');
	...
	{
		$('#formCartasatestados').find(':text, :password, :file, textarea').val('');
		$('#formCartasatestados').find(':radio, :checkbox').attr("checked",false);
		$('#formCartasatestados').find('select').val('');
		clear_form_erros();
	}
	...
	function cadastrar()
	{
		$('#modalCadastraCartasatestados .modal-title').html('<strong><i class="fa fa-plus-square"></i>&nbsp;Cadastro de Evento</strong>');
		$('#modalCadastraCartasatestados').modal('show');
		clear_form_erros();
	}
	function editar(id) //Preenche os campos do formulário apartir do DB
	{
		...
		$('#modalCadastraCartasatestados .modal-title').html('<strong><i class="glyphicon glyphicon-edit"></i>&nbsp;Editar Evento</strong>');
		$.ajax(
		{
	...
	$(function()
	{
		$('#modalCadastraCartasatestados').on('hide.bs.modal', function(e)
		{
			clear_inputs_modal();
		});
		$('#formCartasatestados').on('submit', function(event)
		{
			...
			.done(function(data) //success
			{
				...
				if(data.status == 1)
				{
					//alert_success('.msg', data.msg);
					alertify.success('<i class="fa fa-check-circle-o"></i> '+data.msg);
					$('#tableCartasatestados').bootstrapTable('refresh', {url: base_url_controller+'bootstrap_table'});
					$('#modalCadastraCartasatestados').modal('hide');
				}
				else
				{
					var LinkNavTabs = '';
