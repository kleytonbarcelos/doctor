...
<table
	...
	data-sort-name="nomepaciente"
	>
	<thead>
		<tr>
			<th data-checkbox="true"></th>
			<th data-field="nomepaciente" data-sortable="true" data-formatter="formatter_nome">Nome</th>
			<th data-field="convenio_id" data-sortable="true">Convênio</th>
			<th data-field="dataconsulta" data-sortable="true">Nome</th>
...
<script type="text/javascript">
	...
	{
		return '<a href="'+base_url_controller+'editar/'+row.id+'">'+row.nomepaciente+'</a>';
	}
	function formatter_actions(value, row, index)
	{
		return [
			'<a title="Ver Evento" data-toggle="tooltip" class="btn btn-default btn-xs" href="'+base_url_controller+'visualizar/'+row.id+'"><i class="fa fa-vcard"></i></a>',
			'&nbsp;',
			'<a title="Editar Registro" data-toggle="tooltip" class="btn btn-default btn-xs" href="'+base_url_controller+'editar/'+row.id+'"><i class="fa fa-pencil"></i></a>',
			].join('');
	}
	...
	function cadastrar()
	{
		$('#modalCadastraTiss .modal-title').html('<strong><i class="fa fa-plus-square"></i>&nbsp;Cadastro de Evento</strong>');
		$('#modalCadastraTiss').modal('show');
		clear_form_erros();
	}
	function editar(id) //Preenche os campos do formulário apartir do DB
	{
		...
		$('#modalCadastraTiss .modal-title').html('<strong><i class="glyphicon glyphicon-edit"></i>&nbsp;Editar Evento</strong>');
		$.ajax(
		{
	...
	$(function()
	{
		$('#modalCadastraTiss').on('hide.bs.modal', function(e)
		{
			clear_inputs_modal();
		});
		$('#formTiss').on('submit', function(event)
		{
			...
			.done(function(data) //success
			{
				...
				if(data.status == 1)
				{
					//alert_success('.msg', data.msg);
					alertify.success('<i class="fa fa-check-circle-o"></i> '+data.msg);
					$('#tableTiss').bootstrapTable('refresh', {url: base_url_controller+'bootstrap_table'});
					$('#modalCadastraTiss').modal('hide');
				}
				else
				{
					var LinkNavTabs = '';
