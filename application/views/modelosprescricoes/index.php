...
<script type="text/javascript">
	...
	//#######################################################################################################################
	var $table = $('#tableModelosPrescricoes');
	...
	{
		return '<a href="'+base_url_controller+'editar/'+row.id+'">'+row.nome+'</a>';
	}
	...
	function cadastrar()
	{
		$('#modalCadastraModelosPrescricoes .modal-title').html('<strong><i class="fa fa-plus-square"></i>&nbsp;Cadastro de Evento</strong>');
		$('#modalCadastraModelosPrescricoes').modal('show');
		clear_form_erros();
	}
	function editar(id) //Preenche os campos do formulário apartir do DB
	{
		...
		$('#modalCadastraModelosPrescricoes .modal-title').html('<strong><i class="glyphicon glyphicon-edit"></i>&nbsp;Editar Evento</strong>');
		$.ajax(
		{
	...
	$(function()
	{
		$('#modalCadastraModelosPrescricoes').on('hide.bs.modal', function(e)
		{
			clear_inputs_modal();
		});
		$('#formModelosPrescricoes').on('submit', function(event)
		{
			...
			.done(function(data) //success
			{
				...
				if(data.status == 1)
				{
					//alert_success('.msg', data.msg);
					alertify.success('<i class="fa fa-check-circle-o"></i> '+data.msg);
					$('#tableModelosPrescricoes').bootstrapTable('refresh', {url: base_url_controller+'bootstrap_table'});
					$('#modalCadastraModelosPrescricoes').modal('hide');
				}
				else
				{
					var LinkNavTabs = '';
